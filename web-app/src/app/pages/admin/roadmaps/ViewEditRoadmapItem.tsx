...
import { useUpdateRoadmapItem } from '@feature/roadmap/hooks';
import {
  roadmapItemSchema,
  type Roadmap,
  type RoadmapItem,
...
export function RoadmapItemEditModal({ roadmap, item, isOpen, onClose }: RoadmapItemEditModalProps) {
  const { mutate: updateItem, isPending } = useUpdateRoadmapItem();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  }, [item?.item_text, item?.sort_order, roadmap.id, reset]);
        ...
        <LayoutUI.Column gap="gap-4" className="px-5 py-5 sm:px-6 sm:py-6">
          <DialogUI.DialogHeader className="space-y-1 text-left">
            ...
            <DialogUI.DialogDescription className="text-sm text-black/60">
              Update milestone for {roadmap.title}
          ...
          <FormUI.FormRoot onSubmit={handleSubmit(onSubmit)} gap="sm">
            <LayoutUI.Column gap="gap-5">
              <FormUI.FormField id="edit_item_text" label="Item Text" error={errors.item_text?.message} tone="inverse">
                <Textarea id="edit_item_text" {...register('item_text')} rows={5} />
              </FormUI.FormField>

              <LayoutUI.Container className="max-w-xs">
                <FormUI.FormField id="edit_sort_order" label="Sort Order" error={errors.sort_order?.message} tone="inverse">
                  <Input id="edit_sort_order" type="number" {...register('sort_order')} hasError={Boolean(errors.sort_order)} tone="inverse" />
                </FormUI.FormField>
              </LayoutUI.Container>

              <FormUI.FormFooter align="end" gap="md" flush className="border-t border-black/10">
                <Button type="button" onClick={handleClose} variant="destructive" size="form">
                  Cancel
                </Button>
