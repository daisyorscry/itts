...
func (r *partnerRepo) RunInTransaction(ctx context.Context, f func(tx context.Context) error) error {
	return r.db.Run(ctx, f)
}

func (r *partnerRepo) Create(ctx context.Context, m *model.Partner) error {
	if RepoTracer != nil {
		defer RepoTracer.StartDatastoreSegment(ctx, "partners", "Create")()
	}
	return r.db.Get(ctx).Create(m).Error
}
func (r *partnerRepo) GetByID(ctx context.Context, id string) (*model.Partner, error) {
	if RepoTracer != nil {
		defer RepoTracer.StartDatastoreSegment(ctx, "partners", "GetByID")()
	}
...
func (r *partnerRepo) Update(ctx context.Context, m *model.Partner) error {
	if RepoTracer != nil {
		defer RepoTracer.StartDatastoreSegment(ctx, "partners", "Update")()
	}
	return r.db.Get(ctx).Save(m).Error
}
func (r *partnerRepo) Delete(ctx context.Context, id string) error {
	if RepoTracer != nil {
		defer RepoTracer.StartDatastoreSegment(ctx, "partners", "Delete")()
	}
	return r.db.Get(ctx).Delete(&model.Partner{}, "id = ?", id).Error
}
func (r *partnerRepo) List(ctx context.Context, p ListParams) (*PageResult[model.Partner], error) {
	...
	q, err := ApplyListQuery(r.db.Get(ctx).Model(&model.Partner{}), &p, searchable, sorts)
	if err != nil {
		return nil, err
	}
